...
def run_client():
    ...
    def show_login_window():
        def login():
            if name_in.get() == os.getenv('USER') and pass_in.get() == os.getenv('PASS'):
                ...
                window.destroy()
                has_access = True
                client.publish('auth/login', f'Authorization for client {os.getenv("TERMINAL")}')
            else:
                placeholder.config(text='Invalid password or name.')
                client.publish('auth/login', f'Authorization error {os.getenv("TERMINAL")}')

        window = tk.Tk()
        window.title('Log in')
        window.resizable(False, False)

        placeholder = tk.Label(window, text='Use your data to log in.')
    ...
    def show_client_window():
        ...
        terminal_label = tk.Label(window, text=f'{os.getenv("TERMINAL")} connected.')
        ...
        def publish():
            ...
            if card_id.isdigit():
                ...
                terminal_label.config(text='Card ID has been sent.')
            else:
                terminal_label.config(text='Card ID has to be only a number.')
    ...
    def on_connect(client, userdata, flags, rc):
        ...
        if rc != 0:
            print('Error has been occurred. Reload script.')
            sys.exit(0)
